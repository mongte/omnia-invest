...
> **원문**: [Harness engineering: leveraging Codex in an agent-first world | OpenAI](https://openai.com/index/harness-engineering/)  
...
엔지니어가 작업을 설명하고, 에이전트를 실행하고, pull request를 열도록 허용하는 등 사람은 거의 전적으로 프롬프트를 통해 시스템과 상호작용합니다. PR을 완료하기 위해, Codex에게 로컬에서 자체 변경 사항을 검토하고, 로컬 및 클라우드에서 특정 에이전트 검토를 추가로 요청하고, 사람이나 에이전트가 제공한 피드백에 응답하고, 모든 에이전트 검토자가 만족할 때까지 반복하도록 지시합니다(사실상 랄프 위검 루프(Ralph Wiggum Loop)임). Codex는 사람이 CLI에 복사하여 붙여넣지 않아도 표준 개발 도구(`gh`, 로컬 스크립트, 리포지터리 내장 스킬)를 직접 사용하여 컨텍스트를 수집합니다.
...
### Codex가 Chrome DevTools MCP로 앱을 구동하여 작업을 검증하는 흐름

![Codex drives the app with Chrome DevTools MCP to validate its work](./images/harness-engineering/img-000.png)

> **흐름 설명:**
> 1. **Codex** → APP: 대상 선택 + 콘솔 초기화
> 2. **Codex** → APP: **BEFORE** 스냅샷 촬영
> 3. **Codex** → APP: UI 경로 트리거
> 4. APP → **Chrome DevTools**: 런타임 이벤트 (상호작용 중)
> 5. **Codex** → APP: **AFTER** 스냅샷 촬영
> 6. **Codex** → APP: 수정사항 적용 + 재시작
> 7. **LOOP UNTIL CLEAN**: 검증 재실행

---

관측 가능성 툴링에서도 마찬가지였습니다. 로그, 메트릭, 추적은 각 워크트리에 대해 일시적으로 유지되는 로컬 관측 가능성 스택을 통해 Codex에 노출됩니다. Codex는 해당 앱의 완전히 격리된 버전(로그와 메트릭 포함)에서 작업하며, 작업이 완료되면 로그와 메트릭은 삭제됩니다. 에이전트는 LogQL로 로그를 쿼리하고 PromQL로 메트릭을 쿼리할 수 있습니다.

### Codex에 완전한 관측 가능성 스택 제공

![Giving Codex a full observability stack](./images/harness-engineering/img-001.png)

> **아키텍처 설명:**
>
> ```
> APP ──(LOGS HTTP, OTLP METRICS, OTLP TRACES)──→ VECTOR ──(Fan out local)──┐
>                                                                             │
>                          ┌─────────────────────────────────────────────────┘
>                          ▼
>              ┌─── Observability Stack Services ───┐
>              │  Victoria Logs  → LogQL API        │
>              │  Victoria Metrics → PromQL API     │──→ CODEX ──(UI Journey)──→ Test
>              │  Victoria Traces → TraceQL API     │      │                      │
>              └────────────────────────────────────┘      │                      │
>                                                          ▼                      │
>              Query, Correlate, Reason ←──────────── CODEBASE ←── Re-run workload
>              Implement change (PR)
>              Restart app
>                          ↕
>                    Feedback loop
...
이러한 컨텍스트가 주어지면 "서비스 시작이 800ms 이내에 완료"되도록 하거나 "이 네 가지 핵심 사용자 여정에서 어떤 스팬도 2초를 초과하지 않도록" 하는 등의 프롬프트를 다루기 쉬워집니다.
...
### 에이전트 지식의 한계: Codex가 볼 수 없는 것은 존재하지 않는다

![The limits of agent knowledge: What Codex can't see doesn't exist](./images/harness-engineering/img-003.png)
...
> │                    CODEX'S KNOWLEDGE                            │
> │                                                                 │
> │              Unseen knowledge to Codex         ◀── Encode into  │
> │                                                    codebase     │
> │                                                    as markdown  │
> └─────────────────────────────────────────────────────────────────┘
>         ▲               ▲                  ▲
...
>   │           │  │             │  │               │
>   │ This doc  │  │ We will     │  │ Ryan is       │
>   │ outlines  │  │ follow      │  │ responsible   │
>   │ our       │  │ @PaulM's    │  │ for the       │
>   │ approach  │  │ guidance on │  │ overall       │
>   │ to feature│  │ security    │  │ architectural │
>   │ prioriti- │  │ posture     │  │ direction of  │
>   │ zation and│  │ from now on.│  │ the system.   │
>   │ planning. │  │             │  │               │
>   └───────────┘  └─────────────┘  └───────────────┘
> ```
...
### 명시적 교차 경계가 있는 레이어드 도메인 아키텍처

![Layered domain architecture with explicit cross-cutting boundaries](./images/harness-engineering/img-005.png)

> **아키텍처 다이어그램 설명:**
>
> 각 비즈니스 도메인 내에서(예: 앱 설정) 코드는 고정된 레이어 집합을 통해서만 "전달"할 수 있습니다. 교차되는 문제(인증, 커넥터, 텔레메트, 기능 플래그)는 하나의 명시적인 인터페이스인 `Providers`를 통해 유입됩니다. 그 외의 모든 것은 허용되지 않으며 기계적으로 강제 적용됩니다.
>
> ```
>                    ┌──────────┐
>                    │  Utils   │
...
대신, "황금 원칙"이라는 것을 리포지터리에 직접 인코딩하고 정기적인 정리 프로세스를 구축하기 시작했습니다. 실행에서도 코드베이스의 가독성과 일관성을 유지하도록 설계된 규칙입니다. 예를 들어:
...
이것은 **가비지 컬렉션**처럼 작동합니다. 기술 부채는 고금리 대출과 같아서, 이자가 쌓여 고통스럽게 한꺼번에 갚는 것보다 조금씩 꾸준히 갚아나가는 편이 훨씬 낫습니다. 사람의 취향은 한 번 포착되면 코드의 모든 라인에 지속적으로 반영됩니다. 이렇게 하면 나쁜 패턴이 코드베이스에 며칠 또는 몇 주 동안 퍼지지 않도록 매일 포착하고 해결할 수 있습니다.
...
- [모델에서 에이전트까지: 응답 API에 컴퓨터 환경 제공](https://openai.com/index/from-models-to-agents/) — 엔지니어링, 2026년 3월 11일
- [Codex 사용 한도 그 이상으로 Codex 및 Sora에 대한 액세스 확장하기](https://openai.com/index/expanding-access/) — 엔지니어링, 2026년 2월 13일
- [하네스 활용하기: OpenAI가 App Server를 구축한 방법](https://openai.com/index/unlocking-the-codex-harness/) — 엔지니어링, 2026년 2월 4일
