...
**ranking.md** - 순위정보 API (호가잔량상위, 호가잔량급증, 잔량율급증, 거래량급증, 전일대비등락률상위, 거래대금상위, 외국인기관매매상위) 상세 스펙.
...
**short_lending.md** - 대차거래 API (대차거래추이, 대차거래상위10종목) 상세 스펙.

**chart.md** - 차트 API (ka10081 주식일봉차트조회). 600건/회, 연속조회 지원. ohlcv_daily 백필 및 일일 수집에 사용.
...
| ka10081 | 주식일봉차트 | `/api/dostk/chart` | 지원 |
| ka10032 | 거래대금상위 | `/api/dostk/rkinfo` | 지원 |
| ka90009 | 외국인기관매매상위 | `/api/dostk/rkinfo` | 지원 |
| ka10068 | 대차거래추이 | `/api/dostk/slb` | 지원 |
| ka10069 | 대차거래상위10종목 | `/api/dostk/slb` | 지원 |
