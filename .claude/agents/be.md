...
bash $SCRIPT add-comment <PROJECT_ID> <TASK_ID> agent-be-1 "[작업 시작] IN_PROGRESS 전환. 구현 계획: <접근 방법 요약>"
...
### 5단계: 완료 보고 및 리뷰 요청
```bash
bash $SCRIPT add-comment <PROJECT_ID> <TASK_ID> agent-be-1 "[작업 완료] 변경 파일: <파일 목록>. 수락 기준 달성: <각 기준별 확인 결과>. 자기 검토 완료. QA 검증 요청."
