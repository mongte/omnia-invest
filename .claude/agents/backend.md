...
bash $SCRIPT add-comment <PROJECT_ID> <TASK_ID> agent-be-1 "[작업 시작] IN_PROGRESS 전환. 구현 계획: 1) <구체적 작업 항목>, 2) <구체적 작업 항목>, 3) <구체적 작업 항목>"
...
```bash
bash $SCRIPT add-comment <PROJECT_ID> <TASK_ID> agent-be-1 "[작업 완료] 변경 컴포넌트: <파일1>, <파일2>, ... 수락 기준 달성: (1) <기준> - <결과> (2) <기준> - <결과> ... 자기 검토 완료. QA 검증 요청."
